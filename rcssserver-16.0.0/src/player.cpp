...
// Look here: Overloaded Constructor to upaded logs slide from played match
// TempPlayer is a TempPlayer that houses the all details that is available with the logs

// Player::Player(Stadium & stadium, Team * team, int number, TempPlayer tempPlayer ): MPObject(stadium, "", "",
...
void Player::place( const PVector & pos, const double & angle, const double & neck, const PVector & vel, const PVector & accel )
{
    M_pos = pos;
    M_angle_body = angle;
    M_angle_neck = neck;
    M_angle_neck_committed = neck;
    M_angle_body_committed = angle;
    M_vel = vel;
    M_accel = accel;
