...
bool Stadium::init()
{
    ...
    if (ServerParam::instance().coachMode() && !ServerParam::instance().coachWithRefereeMode())
    {
        for (std::list<Referee *>::iterator i = M_referees.begin();
             i != M_referees.end();
             ++i)
    ...
    // std::cout <<"M_player_types[ 0 ]:  "<< *(M_player_types[ 0 ]) << std::endl;
    // std::cout <<" PlayerParam::instance().playerTypes():  "<<PlayerParam::instance().playerTypes()<< std::endl;

    for (int i = 1; i < PlayerParam::instance().playerTypes(); i++)
    {
        M_player_types.push_back(new HeteroPlayer());
        // std::cout << "*(M_player_types[i]): " << *(M_player_types[i]) << std::endl;
    }
    ...
    if (M_player_socket.setNonBlocking() == -1 || M_offline_coach_socket.setNonBlocking() == -1 || M_online_coach_socket.setNonBlocking() == -1)
    {
...
void Stadium::checkAutoMode()
{
    ...
    //
    if (playmode() == PM_BeforeKickOff && time() < ((ServerParam::instance().halfTime() * ServerParam::instance().nrNormalHalfs()) + (ServerParam::instance().extraHalfTime() * ServerParam::instance().nrExtraHalfs())))
    {
...
void Stadium::startTeams()
{
    if (playmode() != PM_PlayOn)
    {
        if (M_left_child == 0 && !ServerParam::instance().teamLeftStart().empty())
        {
            M_left_child = startTeam(ServerParam::instance().teamLeftStart());
        }

        if (M_right_child == 0 && !ServerParam::instance().teamRightStart().empty() && teamConnected(LEFT))
        {
...
int Stadium::startTeam(const std::string &start)
{
    if (start.empty())
    {
        return 0;
...
bool Stadium::teamConnected(const Side side)
{
    if (side == LEFT)
    {
...
void Stadium::removeListener(const rcss::Listener *listener)
{
...
void Stadium::createObjects()
{
    // Look here: Check when Create Object is called?
    std::cout << "@Stadium::createObjects " << std::endl;
    ...
    // Set Score points for the respective teams
    ...
    //  Look here:
    //
    //  Call a method to load log state
}

Player *Stadium::initPlayer(const char *teamname,
                            ...
                            const rcss::net::Addr &addr)
{
    ...
    if (ServerParam::instance().fixedTeamNameLeft() == teamname || ServerParam::instance().fixedTeamNameRight() == teamname)
    {
...

{
    ...
    {
        if (M_players[r]->team()->enabled() && M_players[r]->team()->name() == teamname && M_players[r]->unum() == unum)
        {
            /* It's same. */
                         ...
                         const rcss::net::Addr &addr)
{
    ...
    if (!M_team_l->name().empty() && M_team_l->name() == teamname)
    {
        olc = M_team_l->olcoach();
    }
    else if (!M_team_r->name().empty() && M_team_r->name() == teamname)
    {
...
void Stadium::step()
{
    ...
    else if (playmode() == PM_AfterGoal_Right || playmode() == PM_AfterGoal_Left || playmode() == PM_OffSide_Right || playmode() == PM_OffSide_Left || playmode() == PM_Illegal_Defense_Left || playmode() == PM_Illegal_Defense_Right || playmode() == PM_Foul_Charge_Right || playmode() == PM_Foul_Charge_Left || playmode() == PM_Foul_Push_Right || playmode() == PM_Foul_Push_Left || playmode() == PM_Back_Pass_Right || playmode() == PM_Back_Pass_Left || playmode() == PM_Free_Kick_Fault_Right || playmode() == PM_Free_Kick_Fault_Left || playmode() == PM_CatchFault_Right || playmode() == PM_CatchFault_Left)
    {
    ...
    if (stoppageTime() == 0 && time() > 0 && ServerParam::instance().nrNormalHalfs() > 0 && time() % (ServerParam::instance().nrNormalHalfs() * ServerParam::instance().halfTime()) == 0)
    {
...
void Stadium::turnMovableObjects()
{
...
void Stadium::incMovableObjects()
{
    std::random_shuffle(M_movable_objects.begin(),
                        M_movable_objects.end(),
                        irand);
    ...
    if (M_ball_catcher)
    {
        ...
        PVector rpos = PVector::fromPolar(M_ball_catcher->size() + ServerParam::instance().ballSize(),
                                          M_ball_catcher->angleBodyCommitted());
        M_ball->moveTo(M_ball_catcher->pos() + rpos);
    }
}

void Stadium::sendDisp()
{
...
Stadium::ballPosInfo()
{
    static RArea g_l(PVector((-ServerParam::PITCH_LENGTH - ServerParam::GOAL_DEPTH) * 0.5 - ServerParam::instance().ballSize(),
                             0.0),
                     PVector(ServerParam::GOAL_DEPTH,
                             ServerParam::instance().goalWidth() + ServerParam::instance().goalPostRadius()));
    static RArea g_r(PVector((+ServerParam::PITCH_LENGTH + ServerParam::GOAL_DEPTH) * 0.5 + ServerParam::instance().ballSize(),
                             0.0),
                     PVector(ServerParam::GOAL_DEPTH,
                             ServerParam::instance().goalWidth() + ServerParam::instance().goalPostRadius()));

    static RArea pt(PVector(0.0, 0.0),
                    PVector(ServerParam::PITCH_LENGTH + ServerParam::instance().ballSize() * 2,
                            ServerParam::PITCH_WIDTH + ServerParam::instance().ballSize() * 2));

    if (g_l.inArea(M_ball->pos()))
        return BPI_GoalL;
    if (g_r.inArea(M_ball->pos()))
        return BPI_GoalR;
    if (!pt.inArea(M_ball->pos()))
        return BPI_OutOfField;
    return BPI_InField;
}

void Stadium::placePlayersInField()
{
    //  Look here: Check if this is called when formation is created.
    //  This method is called every cycle. Why?
    // std::cerr << "@ placePlayersInField ";
    // std::cerr << std::endl;

    static const RArea fld(PVector(0.0, 0.0),
                           PVector(ServerParam::PITCH_LENGTH + ServerParam::PITCH_MARGIN * 2.0,
                                   ServerParam::PITCH_WIDTH + ServerParam::PITCH_MARGIN * 2.0));
...
void Stadium::kickOff()
{
    ...
    if (SP.halfTime() <= 0 || (time() <= normal_time && time() % SP.halfTime() == 0) || (time() > normal_time && SP.extraHalfTime() > 0 && (time() - normal_time) % SP.extraHalfTime() == 0))
    {
        ...
        if (half_time <= 0 || (current_time / half_time) % 2 == 0)
        {
            if (SP.nrNormalHalfs() >= 0 && time() < SP.halfTime() * SP.nrNormalHalfs())
            {
                recoveryPlayers();
            }
...
void Stadium::callHalfTime(const Side kick_off_side,
                           const int half_time_count)
{
    ...
    else
    {
        ...
        M_time = ServerParam::instance().halfTime() * ServerParam::instance().nrNormalHalfs() + ServerParam::instance().extraHalfTime() * extra_count;
    }
...
void Stadium::callFoul(const Side side,
                       PVector pos)
{
    //  Look here: Check if this is called when formation is created.

    std::cerr << "@ Stadium::callFoul ";
    std::cerr << std::endl;
...
void Stadium::dropBall(PVector pos)
{
    //  Look here: Check if this is called when formation is created.

    std::cerr << "@ Stadium::dropBall ";
    std::cerr << std::endl;
    M_ball_catcher = static_cast<const Player *>(0);
...
void Stadium::discardPlayer(const Side side,
                            const int unum)
{
...
void Stadium::punishFoulPlay(const Side side,
                             const int unum)
{
    Player *p = getPlayer(side, unum);
    if (!p || !p->isEnabled())
...
void Stadium::yellowCard(const Side side,
                         const int unum)
{
    Player *p = getPlayer(side, unum);
    ;
...
void Stadium::redCard(const Side side,
                      const int unum)
{
    Player *p = getPlayer(side, unum);
    ;
...
void Stadium::score(const Side side)
{
    if (side == LEFT && M_team_l->enabled())
    {
        M_team_l->incPoint();
    }

    if (side == RIGHT && M_team_r->enabled())
    {
        M_team_r->incPoint();
    }
}

void Stadium::penaltyScore(const Side side,
                           const bool scored)
{
    if (side == LEFT && M_team_l->enabled())
    {
...
void Stadium::penaltyWinner(const Side side)
{
    if (side == LEFT)
    {
...
void Stadium::placeBall(const Side kick_off_side,
                        const PVector &pos)
{
    M_ball->moveTo(pos,
                   PVector(0.0, 0.0),
...
void Stadium::placeBall(const PlayMode pm,
                        const Side kick_off_side,
                        const PVector &pos)
{
    placeBall(kick_off_side, pos);

    if (Referee::isPenaltyShootOut(playmode()) && (pm == PM_PlayOn || pm == PM_Drop_Ball))
    {
...
void Stadium::moveBall(const PVector &pos,
                       const PVector &vel)
{
    M_ball->moveTo(pos,
                   vel,
...
bool Stadium::movePlayer(const Side side,
                         const int unum,
                         const PVector &pos,
                         const double *ang,
                         const PVector *vel)
...
bool Stadium::movePlayerRLE(const Side side,
                         const int unum,
                         const PVector &pos,
                         const double *ang,
                         const double *neck,
                         const PVector *vel)
{
    ...
    double new_angle = player->angleBodyCommitted();
    double new_neck = player->angleNeckCommitted();
    PVector new_vel = player->vel();
    PVector new_accel = player->accel();
...
void Stadium::changePlayMode(const PlayMode pm)
{
    ...
    if (pm == PM_KickOff_Left || pm == PM_KickIn_Left || pm == PM_FreeKick_Left || pm == PM_IndFreeKick_Left || pm == PM_CornerKick_Left || pm == PM_GoalKick_Left || pm == PM_Illegal_Defense_Right)
    {
        M_kick_off_side = LEFT;
    }
    else if (pm == PM_KickOff_Right || pm == PM_KickIn_Right || pm == PM_FreeKick_Right || pm == PM_IndFreeKick_Right || pm == PM_CornerKick_Right || pm == PM_GoalKick_Right || pm == PM_Illegal_Defense_Left)
    {
...
void Stadium::assignPlayerTypes()
{
    if (M_team_l->enabled())
    {
...
void Stadium::substitute(const Player *player,
...
void Stadium::broadcastSubstitution(const int side,
                                    const int unum,
                                    const int player_type)
{
    ...
    for (int i = 0; i < MAX_PLAYER * 2; ++i)
    {
        if (!M_players[i]->isEnabled())
            continue;
        if (M_players[i]->version() < 7.0)
            continue;

        if ((side == LEFT && M_players[i]->team() == M_team_l) || (side == RIGHT && M_players[i]->team() == M_team_r))
        {
    ...
    // tell coaches
    if (M_olcoaches[1] && M_olcoaches[1]->assigned() && M_olcoaches[1]->version() >= 7.0)
    {
        if (side == RIGHT)
        {
    ...
    if (M_olcoaches[0] && M_olcoaches[0]->assigned() && M_olcoaches[0]->version() >= 7.0)
    {
        if (side == LEFT)
        {
...
bool Stadium::changePlayerToGoalie(const Player *player)
{
    if (player->side() == LEFT)
    {
...
void Stadium::broadcastChangePlayerToGoalie(const Player *player)
{
    char msg[64];
    ...
    for (int i = 0; i < MAX_PLAYER * 2; ++i)
    {
        if (M_players[i]->isEnabled() && M_players[i]->version() >= 16.0)
        {
    ...
    for (int i = 0; i < 2; ++i)
    {
        if (M_olcoaches[i] && M_olcoaches[i]->assigned() && M_olcoaches[i]->version() >= 16.0)
        {
...
void Stadium::collisions()
{
    ...
    do
    {
        ...
        for (PlayerCont::iterator it = M_players.begin();
             ...
             ++it)
        {
            if ((*it)->isEnabled() && (*it) != M_ball_catcher && M_ball->pos().distance2((*it)->pos()) < std::pow(M_ball->size() + (*it)->size(), 2))
            {
                col = true;
                (*it)->collidedWithBall();
                for_each(M_referees.begin(), M_referees.end(),
        ...
        for (std::size_t i = 0; i < SIZE - 1; ++i)
        {
            ...
            {
                if (M_players[i]->isEnabled() && M_players[j]->isEnabled() && M_players[i]->pos().distance2(M_players[j]->pos()) < std::pow(M_players[i]->size() + M_players[j]->size(), 2))
                {
                    col = true;
...
void Stadium::calcBallCollisionPos(Player *p)
{
    if (playmode() == PM_PlayOn)
    {
...
void Stadium::calcCollisionPos(MPObject *a,
                               MPObject *b)
{
    if (a == NULL || b == NULL)
    {
    ...
    PVector bpos = b->pos();
    {
        ...
        if (a->pos() == b->pos() || ((a->pos() - mid).r() < EPS && (b->pos() - mid).r() < EPS))
        {
        ...
        while (apos.distance2(bpos) < collision_dist2 && count < 10)
        {
...
void Stadium::setPlayerState(const Side side,
                             const int unum,
                             const int state)
{
    if (unum < 1 || MAX_PLAYER < unum)
...
void Stadium::kickTaken(const Player &kicker,
                        const PVector &accel)
{
    M_ball_catcher = static_cast<const Player *>(0);
...
void Stadium::failedKickTaken(const Player &kicker)
{
    for_each(M_referees.begin(), M_referees.end(),
             Referee::doFailedKickTaken(kicker));
}

void Stadium::tackleTaken(const Player &tackler,
                          const PVector &accel,
                          const bool foul)
{
    M_ball_catcher = static_cast<const Player *>(0);
...
void Stadium::failedTackleTaken(const Player &tackler,
                                const bool foul)
{
    for_each(M_referees.begin(), M_referees.end(),
...
void Stadium::ballCaught(const Player &catcher)
{
    if (!Referee::isPenaltyShootOut(playmode()))
    {
    ...
    if (playmode() == PM_FreeKick_Left || playmode() == PM_FreeKick_Right)
    {
        M_ball_catcher = &catcher;
    }
}

void Stadium::ballPunched(const Player &catcher,
                          const PVector &accel)
{
    M_ball->push(accel);
...
void Stadium::sendTeamGraphic(const Side side,
...
void Stadium::removeDisconnectedClients()
{
...
void Stadium::sendRefereeAudio(const char *msg)
{
...
void Stadium::sendPlayerAudio(const Player &player,
                              const char *msg)
{
    std::random_shuffle(M_listeners.begin(), M_listeners.end(),
                        irand); //rcss::random::UniformRNG::instance() );
...
void Stadium::sendCoachAudio(const Coach &coach,
                             const char *msg)
{
    std::random_shuffle(M_listeners.begin(), M_listeners.end(),
                        irand); //rcss::random::UniformRNG::instance() );
...
void Stadium::sendCoachStdAudio(const OnlineCoach &coach,
...
void Stadium::doRecvFromClients()
{
    ...
    // delayed effects
    //
    if (s_time != M_time && s_stoppage_time != M_stoppage_time)
    {
        s_time = M_time;
...
/*
    Update Ball Parameter given to the input of the Simulator Server. 
...
*/
bool Stadium::updatePlayersParam()
{
    ...
    if (f)
    {
        ...
        if (!j["players"].empty())
        {
            ...
            // std::cout << "Getting Players:  " << j["players"]["player"] << std::endl;
            // }
        ...
        {
            std::cout << "Empty Players" << std::endl;
        }
    }
    else
    {
...
void Stadium::doSendSenseBody()
{
    ...
    for (PlayerCont::iterator it = M_remote_players.begin();
         ...
         ++it)
    {
        if ((*it)->isEnabled() && (*it)->connected())
        {
            (*it)->sendBody();

            if (((*it)->side() == LEFT && ServerParam::instance().fullstateLeft()) || ((*it)->side() == RIGHT && ServerParam::instance().fullstateRight()))
            {
                (*it)->sendFullstate();
            }
        }
...
void Stadium::doSendSynchVisuals()
{
    struct timeval tv_start, tv_end;
...
void Stadium::doSendCoachMessages()
{
    ...
    if (M_coach->assigned() && M_coach->isEyeOn())
    {
...
bool Stadium::doSendThink()
{
    ...
    do
    {
        ...
        for (int i = 0; i < MAX_PLAYER * 2; ++i)
        {
            if (wait_players[i] && M_players[i]->connected() && !M_players[i]->doneReceived() && M_players[i]->isEnabled())
            {
                done = DS_FALSE;
                break;
            }
        ...
        for (int i = 0; i < 2; ++i)
        {
            if (wait_coach[i] && M_olcoaches[i]->connected() && !M_olcoaches[i]->doneReceived() && M_olcoaches[i]->assigned())
            {
                done = DS_FALSE;
                break;
            }
        ...
        double time_diff = (static_cast<double>(tv_now.tv_sec) + static_cast<double>(tv_now.tv_usec) / 1000000) - (static_cast<double>(tv_start.tv_sec) + static_cast<double>(tv_start.tv_usec) / 1000000);
        time_diff *= 1000;
    ...
    if (M_logger.isTextLogOpen() && ServerParam::instance().logTimes())
    {
        char buf[32];
...
{
    recv_from_clients(M_remote_players); // Remote Client communication every cycle?
    ...
    while (1)
    {
        ...
        int len = M_player_socket.recv(message, MaxMesg, cli_addr); // this is entirely something else.

        if (len > 0)
        {
            //  Look here: Enable the and check the message? This is where information is received from client.
            // std::cerr << "Got @ udp_recv_message: ";
...
void Stadium::parsePlayerInit(const char *message,
...
bool Stadium::parseMonitorInit(const char *message,
                               const rcss::net::Addr &addr)
{
    double ver = 1.0;
    if (!std::strncmp(message, "(dispinit)", 10) || std::sscanf(message, " ( dispinit version %lf ) ", &ver) == 1)
    {
        if (ServerParam::instance().maxMonitors() > 0 && static_cast<int>(M_monitors.size()) >= ServerParam::instance().maxMonitors())
        {
...
{
    const bool allow_coach = (ServerParam::instance().coachMode() || ServerParam::instance().coachWithRefereeMode());
...
bool Stadium::parseCoachInit(const char *message,
                             const rcss::net::Addr &cli_addr)
{
...
void Stadium::sendToPlayer(const char *msg,
                           const rcss::net::Addr &cli_addr)
{
...
void Stadium::sendToCoach(const char *msg,
                          const rcss::net::Addr &cli_addr)
{
...
void Stadium::sendToOnlineCoach(const char *msg,
                                const rcss::net::Addr &cli_addr)
{
...
void Stadium::finalize(const std::string &msg)
{
